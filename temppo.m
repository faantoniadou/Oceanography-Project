...
temp = ncread('alltemp.nc','votemper');
sst = temp([290:318],[210:253],:,:);
...
for i = 1:size(sst,1)
    for j = 1:size(sst,2)
        for k = 1:size(sst,3)
            for l = 1:size(sst,4)
                sstemp = sst(:,:,:,l);
                %sstemp = flip((triu(flip(sstemp),-10)));
                %sstemp = triu(sstemp,-80);
                %sstemp(sstemp==0) = NaN;
...
%title('Annual Mean Sea Surface Temperature in the river Po estuary region');
...
polynomial2 = polyfit(datenum(w),yearlyavg_sst, 4);
